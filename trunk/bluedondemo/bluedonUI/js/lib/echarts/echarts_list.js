...
$.fn.chartsObj = function(option){
    ...
    require([ 'echarts', 'echarts/chart/'+tp]);
		...
		case 'line1'://有背景虚线
			...
			$.getJSON(jsonA, function (jsonArray) { 
				...
				for(var key in jsonArray.seriesList){
					tt[key] = jsonArray.seriesList[key].name;
					if(option.lineType){
						seriesData[key] = {data:jsonArray.seriesList[key].dataNumber,type:tp,smooth:false,itemStyle: {normal: {areaStyle: {type: 'default'}}},name:jsonArray.seriesList[key].name};
						}else{
						seriesData[key] = {data:jsonArray.seriesList[key].dataNumber,type:tp,name:jsonArray.seriesList[key].name};
					}
				};
			}); 
