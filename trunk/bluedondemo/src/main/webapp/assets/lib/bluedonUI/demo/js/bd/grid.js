       ...
       var getData  = function(data){
		...
		if (opt.sColumnsUrl) {
			$.ajax({
					...
					data.push({
						 display: '当前流量(MB)', name: 'sFrameliu',minWidth: 150,
						  render: function (item)
							{
								var h = [];
								if (parseInt(item.sFrameZj) < 1000) {h.push( "<font class='green'>上行:"+item.sFrameZj+"</font>");
								}else{h.push( "<font class='red'>上行:"+item.sFrameZj+"</font>");}
				...
				if(opt.del == true || opt.view == true || opt.edit == true||opt.showhf == true)
				{
					data.push({
